...
const uint32_t colecovision_palette_rgb[0x100] = {
...
uint32_t colecovision_4_phase[256] = {
...
uint32_t _colecovision_4_phase_pal[] = {
...
static void gen_ntsc_pal_tables() {
    ...
    printf("uint32_t colecovision_4_phase[256] = {\n");
    int cc_width = 4;
    ...
    make_yuv_palette("_colecovision",rgb,256);
}
...
// big mem req
//uint8_t colecovision_videodata[256*240] = {0};   // larger than it needs to be at 60k

uint8_t* colecovision_videodata = 0;               // dynamically allocated
uint8_t* _colecovision_rom;

const char* _colecovision_ext[] = {
    "col",
    0
};
...
class EmuColecovision : public Emu {
    uint8_t** _lines;
public:
    EmuColecovision(int ntsc) : Emu("colecovision",256,240,ntsc,(16 | (1 << 8)),4,EMU_COLECOVISION)    // audio is 16bit
    {
        _lines = 0;
        _ext = _colecovision_ext;
        _help = _colecovision_help;
    }
    ...
    virtual int info(const string& file, vector<string>& strs)
    {
        ...
        strs.push_back("");
        if (ext == "col")
            strs.push_back(::to_string((len + 0x3FF)/1024) + "k Colecovision");
        else
    ...
    void init_screen() {
        printf("init_screen\n");
        colecovision_videodata = new uint8_t[256*240];
        bitmap.data = colecovision_videodata + 24*256;
        ...
        const uint8_t* s = colecovision_videodata;
        for (int y = 0; y < 240; y++) {
            _lines[y] = (uint8_t*)s;
            s += 256;
        }
    ...
    void clear_screen()
    {
        if (colecovision_videodata)
            memset(colecovision_videodata,0,256*240);
    }
    ...
    virtual int insert(const std::string& path, int flags, int disk_index)  {
    	...
    	// Make sure we have a bios.
    	f = fopen("/colecovision/coleco.rom", "rb");
    	if (!f) {
    		printf("Error: Coleco BIOS not found.\nMake sure the file coleco.rom is on there.\n");
		} else {
			int size = fread(cart.bios,1,8192,f);
			fclose(f);
			printf("Read the Coleco BIOS: %d bytes.\n",size);
		}
		...
		if (!f) {
			...
			int size = fread(cart.rom,1,32768,f);
...
/*		uint8_t buf[16];
		...
		_colecovision_rom = map_file(path.c_str(),len);
		
		cart.rom = _colecovision_rom; */
    ...
    const uint32_t _common_colecovision[16] = {
        0,  // msb
        0,
        0,
        INPUT_START<<8, // PLUS
    ...
    const uint32_t _classic_colecovision[16] = {
        INPUT_RIGHT,    // RIGHT
        INPUT_DOWN,     // DOWN
        0,              // LEFT_TOP
        INPUT_PAUSE<<8, // MINUS
    ...
    const uint32_t _generic_colecovision[16] = {
    ...
    virtual const uint32_t* ntsc_palette() { return colecovision_4_phase; };
    virtual const uint32_t* pal_palette() { return _colecovision_4_phase_pal; };
    virtual const uint32_t* rgb_palette() { return colecovision_palette_rgb; };
...
Emu* NewColecovision(int ntsc)
{
    return new EmuColecovision(ntsc);
}
