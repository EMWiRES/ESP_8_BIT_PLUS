...
void gui_update()
{
    ...
    // Consume all Bluetooth packets to prevent lag
    // on the 8BitDo zero 2.
    while(n) {
...
static void bitDoFC30(const uint8_t* j,int len) {
	int pad = 0;
...
static void bitDoZero2(const uint8_t*j,int len) {
	int pad = 0;
...
void gui_hid(const uint8_t* hid, int len)  // Parse HID event
{
    ...
    switch (hid[1]) {
		...
		case 0x03: bitDoFC30(hid+1,len-1); break;
    	case 0x3F: bitDoZero2(hid+1,len-1); break;
